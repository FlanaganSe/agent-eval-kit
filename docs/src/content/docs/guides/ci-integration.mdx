...
jobs:
  eval:
    ...
    steps:
      ...
      - name: Run evals
        run: pnpm agent-eval-kit run --mode=replay --suite=smoke

      # Or run live with strict fixtures
      - name: Run evals (strict)
        run: pnpm agent-eval-kit run --mode=replay --strict-fixtures --suite=smoke
```
...
This adds a pre-push hook that runs `agent-eval-kit run --mode=replay`. Failed gates block the push.
