...
describe("formatMarkdownReport", () => {
	...
	it("escapes backticks in grader reasons", () => {
		const md = formatMarkdownReport(
			makeRun([
				makeTrial({
					status: "fail",
					...
					grades: [
						{
							pass: false,
							score: 0,
							reason: "Expected `search` tool call",
		...
		expect(md).not.toContain("| Expected `search`");
	});

	it("escapes HTML tags in grader reasons", () => {
		const md = formatMarkdownReport(
			makeRun([
				makeTrial({
					status: "fail",
					...
					grades: [
						{
							pass: false,
							score: 0,
							reason: "Output contained <script>alert</script>",
							graderName: "safety",
						},
					],
				}),
